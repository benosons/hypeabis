...
/**
 * Admin_report_author_active
 *
 * @property Global_lib $global_lib
 * @property CI_Input $input
 * @property Mdl_report $mdl_report
 */
class Admin_report_author_active extends CI_Controller {
  ...
  public function index($search_param = NULL)
  {
    ...
    $pagination_config['total_rows'] = $this->mdl_report->count_author_activity($search_param);
    ...
    $this->data['items'] = $this->mdl_report->all_author_activity($search_param, $pagination_config['per_page'], $this->data['offset']);

    $this->_render('admin/report/author_active');
  }
  ...
  public function export()
  {
    ...
    //get search result
    $data = $this->mdl_report->all_author_activity($search_param);
    $clean_data = [];
    ...
    foreach ($data as $x => $item) {
      array_push($clean_data, [
        ...
        'Terakhir Login' => $item->last_login_at
      ]);
    ...
    $reports = [
      [
        'title' => "Author Aktif - Tidak Aktif",
        'data' => $clean_data
      ]
    ];
    $this->excel->streamMultipleSheet('report_author_aktif_' . date('YmdHis') . '.xls', $reports);
  }
