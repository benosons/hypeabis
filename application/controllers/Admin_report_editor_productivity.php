...
/**
 * Admin_report_editor_productivity
 *
 * @property Global_lib $global_lib
 * @property CI_Input $input
 * @property Mdl_report $mdl_report
 */
class Admin_report_editor_productivity extends CI_Controller {
  ...
  public function index($search_param = NULL)
  {
    ...
    $pagination_config['total_rows'] = $this->mdl_report->count_editor_productivity($search_param);
    ...
    $this->data['items'] = $this->mdl_report->all_editor_productivity($search_param, $pagination_config['per_page'], $this->data['offset']);

    $this->_render('admin/report/editor_productivity');
  }
  ...
  public function export()
  {
    ...
    //get search result
    $data = $this->mdl_report->all_editor_productivity($search_param);
    $clean_data = [];
    ...
    $reports = [
      [
        'title' => "Produktivitas Editor",
        'data' => $clean_data
      ]
    ];
    $this->excel->streamMultipleSheet('report_editor_' . date('YmdHis') . '.xls', $reports);
  }
