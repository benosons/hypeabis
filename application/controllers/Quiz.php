...
class Quiz extends CI_Controller
{
    ...
    private $base_url;
    private $is_admin;
    private $module_name = 'admin_quiz';
    private $data = [];
    ...
    public function __construct()
    {
        ...
        $this->is_admin = $this->session->userdata('admin_logged_in') && $this->uri->segment(1) === 'admin_quiz';
        $this->data = [
        ...
        if ($this->uri->segment(1) === 'user_quiz') {
            $this->mdl_quiz->filter_by_user($this->session->userdata('id_user'));
        }
    }
    ...
    public function index()
    {
        ...
        $pagination_config['total_rows'] = $this->mdl_quiz->without_draft($this->is_admin)->count();
        ...
        $this->data['quizzes'] = $this->mdl_quiz
        ...
        $this->_render($this->load->view('shared/quiz/all', $this->data, TRUE));
    }

    public function view($id_content)
    {
        $this->data['content'] = $this->mdl_quiz->find($id_content) ?: redirect($this->uri->segment(1));
        $this->data['tags'] = $this->mdl_quiz->get_tags($id_content);
        $this->data['questions'] = $this->mdl_quiz->get_questions_with_choices($id_content);

        $this->_render($this->load->view('shared/quiz/view', $this->data, TRUE));
    }
    ...
    public function add()
    {
        ...
        $this->_render($this->load->view('shared/quiz/form', $this->data, TRUE));
    }
    ...
    public function save_add()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $id_content = $this->mdl_quiz->insert_content($content_data, $tags);
            ...
            if (is_null($this->input->post('preview'))) {
                ...
                redirect("quiz/{$id_content}{$name}/" . strtolower(url_title($content_data['title'])) . '?is_preview=1');
            }
        }
        else {
    ...
    public function edit($id_content)
    {
        ...
        $this->data['questions'] = $this->mdl_quiz->get_questions($id_content);
        ...
        $this->data['locked_content_id'] = $this->mdl_quiz->get_locked_content_id($this->session->userdata('id_admin'));
    ...
    {
        $content = $this->mdl_quiz->find($id_content) ?: redirect($this->uri->segment(1));
        ...
        if ($this->form_validation->run()) {
            ...
            $tags = !empty($this->input->post('tags')) ? explode(',', $this->input->post('tags')) : [];
            $this->mdl_quiz->update_content($id_content, $content_data, $tags);
            ...
            if ($content_data['content_pic'] !== $content->content_pic) {
                @unlink("assets/quiz/{$content->content_pic}");
                @unlink("assets/quiz/thumb/{$content->content_pic_thumb}");
                @unlink("assets/quiz/thumb/{$content->content_pic_square}");
            }

            if ($content->content_status !== $content_data['content_status']) {
                $this->load->library('point_lib');
                $point_config = ['trigger_type' => 'add_quiz', 'id_user' => $content->id_user, 'desc' => $content->title];
    ...
    {
        $content = $this->mdl_quiz->find($id_content);
        ...
        if (!is_null($content)) {
            ...
            $questions = $this->mdl_quiz->get_questions_with_choices($id_content);

            $this->mdl_quiz->delete($id_content);
            ...
            foreach ($questions as $question) {
                if (!empty($question->picture)) {
                    @unlink("assets/quiz/question/{$question->picture}");
                }

                foreach ($question->choices as $choice) {
                    if (!empty($choice->picture)) {
                        @unlink("assets/quiz/choice/{$choice->picture}");
                    }
                }
            }
            ...
            $this->session->set_flashdata(
                'message',
                $this->global_lib->generateMessage('Quiz berhasil dihapus.', 'info')
            );
        }
    ...
    public function delete_picture($id_content)
    {
        ...
        if (!is_null($content) && $content->content_status !== '1') {
            ...
            $this->mdl_quiz->remove_picture($id_content);
            ...
            $this->session->set_flashdata(
                'message',
                $this->global_lib->generateMessage('Gambar konten Quiz berhasil dihapus', 'info')
            );
        }
    ...
    public function add_question($id_content)
    {
        ...
        $this->data['max_order_no'] = $this->mdl_quiz->get_max_question_order_no($id_content);
        ...
        $this->_render($this->load->view('shared/quiz/form_question.php', $this->data, TRUE));
    }
    ...
    public function save_add_question($id_content)
    {
        ...
        $max_order_no = $this->mdl_quiz->get_max_question_order_no($id_content);
        ...
        if ($this->form_validation->run()) {
            ...
            $this->mdl_quiz->add_question($id_content, $data);
    ...
    {
        $this->data['question'] = $this->mdl_quiz->find_question($id) ?: redirect($this->uri->segment(1));
        $this->data['content'] = $this->mdl_quiz->find($this->data['question']->id_content);

        if ($this->data['content']->edit_id_admin && $this->data['content']->edit_id_admin != $this->session->userdata('id_admin')) {
            redirect($this->uri->segment(1));
        }

        $this->data['max_order_no'] = $this->mdl_quiz->get_max_question_order_no($this->data['question']->id_content) - 1;
    ...
    {
        $question = $this->mdl_quiz->find_question($id) ?: redirect($this->uri->segment(1));
        $content = $this->mdl_quiz->find($question->id_content) ?: redirect($this->uri->segment(1));

        if ($content->edit_id_admin && $content->edit_id_admin != $this->session->userdata('id_admin')) {
            redirect($this->uri->segment(1));
        }

        $max_order_no = $this->mdl_quiz->get_max_question_order_no($question->id_content) - 1;
        ...
        if ($this->form_validation->run()) {
            ...
            $this->mdl_quiz->update_question($id, $data);
    ...
    {
        $question = $this->mdl_quiz->find_question($id);
        ...
        if (!is_null($question)) {
            ...
            $this->mdl_quiz->delete_question($id);
    ...
    public function delete_question_picture($id)
    {
        ...
        if ($question && $question->picture) {
            ...
            $this->mdl_quiz->update_question($id, ['picture' => NULL]);
    ...
    public function add_choice($id)
    {
        $this->data['question'] = $this->mdl_quiz->find_question($id) ?: redirect($this->uri->segment(1));
        $this->data['content'] = $this->mdl_quiz->find($this->data['question']->id_content) ?: redirect($this->uri->segment(1));

        if ($this->data['content']->edit_id_admin && $this->data['content']->edit_id_admin != $this->session->userdata('id_admin')) {
            redirect($this->uri->segment(1));
        }

        $this->data['max_order_no'] = $this->mdl_quiz->get_max_choice_order_no($id);

        $this->data['heading_text'] = 'Tambah';
        $this->data['submit_url'] = "{$this->base_url}/save_add_choice/{$id}";
        $this->data['form_value'] = $this->session->flashdata('form_value') ?: [
            'order_no' => $this->data['max_order_no'],
            'text' => '',
            'is_answer' => '0',
        ...
        $this->_render($this->load->view('shared/quiz/form_choice.php', $this->data, TRUE));
    }

    public function save_add_choice($id)
    {
        $question = $this->mdl_quiz->find_question($id) ?: redirect($this->uri->segment(1));
        $content = $this->mdl_quiz->find($question->id_content) ?: redirect($this->uri->segment(1));
        ...
        $max_order_no = $this->mdl_quiz->get_max_choice_order_no($id);

        $this->session->set_flashdata('form_value', $this->input->post());
        $this->_set_choice_validation_rules($max_order_no);

        if ($this->form_validation->run()) {
            $picture = $this->_upload_choice_picture("add_choice/{$id}");
            $data = [
                'order_no' => $this->input->post('order_no'),
                'text' => $this->input->post('text'),
                'picture' => $picture,
            ...
            $this->mdl_quiz->add_choice($id, $data);
            ...
            redirect("{$this->uri->segment(1)}/edit_question/{$id}#choices");
        }
        else {
            $this->session->set_flashdata(
                'choice_message',
            ...
            redirect("{$this->uri->segment(1)}/add_choice/{$id}");
        }
    }

    public function edit_choice($id)
    {
        $this->data['choice'] = $this->mdl_quiz->find_choice($id) ?: redirect($this->uri->segment(1));
        $this->data['question'] = $this->mdl_quiz->find_question($this->data['choice']->id_content_quiz_question, FALSE);
        $content = $this->mdl_quiz->find($this->data['question']->id_content) ?: redirect($this->uri->segment(1));

        if ($content->edit_id_admin && $content->edit_id_admin != $this->session->userdata('id_admin')) {
            redirect($this->uri->segment(1));
        }

        $this->data['max_order_no'] = $this->mdl_quiz->get_max_choice_order_no($this->data['question']->id) - 1;

        $this->data['heading_text'] = 'Edit';
        $this->data['submit_url'] = "{$this->base_url}/save_edit_choice/{$id}";
        $this->data['form_value'] = $this->session->flashdata('form_value') ?: (array)$this->data['choice'];

        $this->_render($this->load->view('shared/quiz/form_choice.php', $this->data, TRUE));
    }

    public function save_edit_choice($id)
    {
        $choice = $this->mdl_quiz->find_choice($id) ?: redirect($this->uri->segment(1));
        $question = $this->mdl_quiz->find_question($choice->id_content_quiz_question) ?: redirect($this->uri->segment(1));
        $content = $this->mdl_quiz->find($question->id_content) ?: redirect($this->uri->segment(1));
        ...
        $max_order_no = $this->mdl_quiz->get_max_choice_order_no($choice->id_content_quiz_question) - 1;

        $this->session->set_flashdata('form_value', $this->input->post());
        $this->_set_choice_validation_rules($max_order_no);

        if ($this->form_validation->run()) {
            $picture = $this->_upload_choice_picture("edit_choice/{$id}");
            $data = [
                'order_no' => $this->input->post('order_no'),
                'text' => $this->input->post('text'),
                'picture' => $picture ?: $choice->picture,
                'is_answer' => $this->input->post('is_answer'),
            ];

            $this->mdl_quiz->update_choice($id, $data);
            ...
            redirect("{$this->uri->segment(1)}/edit_question/{$choice->id_content_quiz_question}#choices");
        }
        else {
            $this->session->set_flashdata(
                'choice_message',
            ...
            redirect("{$this->uri->segment(1)}/edit_choice/{$id}");
        }
    }

    public function delete_choice($id)
    {
        $choice = $this->mdl_quiz->find_choice($id);
        ...
        if (!is_null($choice)) {
            ...
            $this->mdl_quiz->delete_choice($id);
    ...
    public function delete_choice_picture($id)
    {
        $choice = $this->mdl_quiz->find_choice($id);

        if ($choice && $choice->picture) {
            $question = $this->mdl_quiz->find_question($choice->id_content_quiz_question) ?: redirect($this->uri->segment(1));
            $content = $this->mdl_quiz->find($question->id_content) ?: redirect($this->uri->segment(1));
            ...
            $this->mdl_quiz->update_choice($id, ['picture' => NULL]);
            ...
            $this->session->set_flashdata(
                'choice_message',
                $this->global_lib->generateMessage('Gambar Pilihan berhasil dihapus.', 'info')
            );
        }
    ...
    private function _toggle_lock_edit($is_lock, $id_content)
    {
        ...
        $locked_content_id = $this->mdl_quiz->get_locked_content_id($this->session->userdata('id_admin'));
        ...
        if ($is_valid_lock || $is_valid_unlock) {
            $this->mdl_quiz->update_content_without_tags($id_content, [
    ...
    private function _set_question_validation_rules($max_order_no)
    {
        ...
        $this->form_validation->set_rules('text', '', "{$base_rule}|required|max_length[500]");
    }

    private function _set_choice_validation_rules($max_order_no)
    {
        $base_rule = 'htmlentities|strip_tags|trim|xss_clean';

        $this->form_validation->set_rules('order_no', '', "{$base_rule}|required|integer|greater_than[0]|less_than_equal_to[{$max_order_no}]");
        $this->form_validation->set_rules('text', '', "{$base_rule}|required|max_length[500]");
        $this->form_validation->set_rules('is_answer', '', "{$base_rule}|required|max_length[500]");
    }
    ...
    private function _upload_content_picture($is_required = FALSE, $fail_redirect_url = NULL)
    {
        ...
        elseif ($is_required) {
            $this->session->set_flashdata(
                'message',
                $this->global_lib->generateMessage('You must upload quiz picture', 'danger')
            );
            redirect("{$this->uri->segment(1)}/{$fail_redirect_url}");
        }
    ...
    private function _upload_question_picture($fail_redirect_url = NULL)
    {
        ...
        if (!empty($filename)) {
            ...
            $config = [
                'upload_path' => './assets/quiz/question',
                'allowed_types' => 'jpg|jpeg|png|gif',
                'max_size' => '12000',
                'max_width' => '8000',
                'max_height' => '8000',
    ...
    private function _upload_choice_picture($fail_redirect_url = NULL)
    {
        $filename = $_FILES['picture']['name'];
        $picture = NULL;
        ...
        if (!empty($filename)) {
            $picture = 'Original_' . date('YmdHisu') . '_' . str_replace('Original_', '', $filename);
            $config = [
                'upload_path' => './assets/quiz/choice',
                'allowed_types' => 'jpg|jpeg|png|gif',
                'max_size' => '12000',
                'max_width' => '8000',
                'max_height' => '8000',
    ...
    {
        $is_admin = $this->uri->segment(1) === 'admin_quiz';
        $data = [
            'content' => $page,
            'type' => $page,
            'css_files' => $this->css,
    ...
    {
        if ($this->uri->segment(1) === 'admin_quiz') {
            if (
        ...
        elseif ($this->uri->segment(1) === 'user_quiz') {
