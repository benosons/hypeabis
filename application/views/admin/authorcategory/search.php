...
<div class="bg-master-lighter padding-20 m-b-20 b-rad-lg">
  <?= form_open('admin_authorcategory/submitSearch', array('role' => 'form', 'class' => 'form-horizontal', 'autocomplete' => 'off')); ?>
          ...
          <div class="col-md-6">
            <select class="full-width select_withsearch" name="category">
              ...
              <? foreach($categories as $item){ ?>
                <option value="<?= $item['id_category']; ?>"  <?= ($item['has_child'] == 1 ? 'disabled="disabled"' : ''); ?> <?= ($search_param['category'] == $item['id_category'] ? "selected" : ""); ?>>
                  <?= $item['category_name']; ?>
                </option>
              <? } ?>
            </select>
      ...
      <div class="row">
        ...
        <div class="col-md-4 m-b-5">  
          <select class="full-width select_nosearch" name="search_by">
            <option value="tbl_user.name" <?= ($search_param['search_by'] == "tbl_user.name" ? "selected" : ""); ?>>Nama penulis</option>
          </select>
        </div>
