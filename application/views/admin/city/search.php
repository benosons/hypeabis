...
<div class="bg-master-lighter padding-20 m-b-20 b-rad-lg">
  <?= form_open('admin_city/submitSearch', array('role' => 'form', 'class' => 'form-horizontal', 'autocomplete' => 'off')); ?>
    ...
    <div id="search_wrapper" <?= (isset($search_param['search_collapsed']) && $search_param['search_collapsed'] == '1' ? 'style="display:none"' : ''); ?>>
      <div class="form-group">
        <div class="row">
          ...
          <div class="col-md-6">
            <select class="full-width select_withsearch" name="province">
      ...
      <div class="form-group">
        <div class="row">
          <label class="col-md-2 control-label text-right sm-text-left m-b-5">City type : </label>
          <div class="col-md-6">
            <select class="full-width select_nosearch" name="city_type">
              <? if($search_param['city_type'] == 'all'){ ?>
                <option value="all" selected>All</option>
              <? } else { ?>
                <option value="all">All</option>
              <? } ?>
              
              <option value="Kabupaten" <?= ($search_param['city_type'] == "Kabupaten" ? "selected" : ""); ?>>District / Kabupaten</option>
              <option value="Kota" <?= ($search_param['city_type'] == "Kota" ? "selected" : ""); ?>>City / Kota</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div class="row">
        ...
        <div class="col-md-4 m-b-5">  
          <select class="full-width select_nosearch" name="search_by">
            <option value="city_name" <?= ($search_param['search_by'] == "city_name" ? "selected" : ""); ?>>City name</option>
          </select>
        </div>
