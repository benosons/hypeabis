            ...
            <div class="card card-transparent">
                <div class="card-body">
                    ...
                    <?php if (isset($competition) && count($competition) > 0) { ?>
                        <?php
                            $is_can_update_competition = false;
                            $mode = null;
                            if(isset($content->id_competition) && $content->id_competition > 0) {
                                $mode = 'edit';
                                foreach ($competition as $comp) {
                                    if ($comp->id_competition == $content->id_competition) {
                                        $is_can_update_competition = true;
                                        break;
                                    }
                                }
                            }
                        ...
                        <div class="d-flex">
                            ...
                            <input type="hidden" id="show_competition_option" value="<?= (count($competition) > 1 ? '1' : '0'); ?>">
                            <input type="hidden" name="is_can_update_competition" id="is_can_update_competition" value="<?= ($is_can_update_competition ? '1' : '0'); ?>"/>
                            <input type="hidden" name="mode" id="mode" value="<?= $mode; ?>"/>
                            <input type="hidden" id="is_join" value="<?= (isset($content->id_competition) && $content->id_competition > 0 ? '1' : '0'); ?>"/>
                        </div>
                        ...
                        <div class="bg-master-lighter padding-20 m-b-20 b-rad-lg">
                            <div class="form-group">
                                <div class="row">
                                    ...
                                    <div class="col-md-8">
                                        ...
                                        <?php } elseif (isset($content) && !is_null($content->id_competition) && $content->id_competition !== $competition[0]->id_competition) { ?>
                                            <div class="alert alert-warning">
                                                Foto ini telah diikutkan pada kompetisi <?php echo $content->competition_title ?>
                                                (<?php echo date('d M Y', strtotime($content->competition_start_date)) ?> - <?php echo date('d M Y', strtotime($content->competition_finish_date)) ?>).
                                            </div>
                                        <?php } else { ?>
                                            <div class="radio radio-complete">
                                                <input type="radio" value="1" name="join_competition"
                                                    id="join_competition1" <?= ($form_value['join_competition'] == '1' ? 'checked="checked"' : ''); ?>>
                                                <label for="join_competition1" calss="text-black">Ya</label>
                                                <input type="radio" value="0" name="join_competition"
                                                    id="join_competition0" <?= ($form_value['join_competition'] == '0' ? 'checked="checked"' : ''); ?>>
                            ...
                            <div id="competition_option_wrapper" <?= (count($competition) > 1 ? '' : 'style="display:none"'); ?>>
                                <div class="form-group">
                                    <div class="row">
                                        <label
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-8">
                                        <a href="javascript:;" onclick="showModal()" class="btn btn-light"><i
                                                class="fa fa-question-circle"></i> Tentang kompetisi <?php echo $competition[0]->title ?>
                                        </a>
                                    </div>
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    function competitionChanged() {
        let mode = $('#mode').val();
        let is_can_update_competition = $('#is_can_update_competition').val();
        let is_join = $('#is_join').val();
        let value = $('[name=join_competition]:checked').val();
        let show_option = $('#show_competition_option').val();

        // console.log('value: ' + value);
        // console.log('mode: ' + mode);
        // console.log('is_join: ' + is_join);
        // console.log('show_option: ' + show_option);
        if(value === '1' || (mode === 'edit' && is_join === '1')) {
        ...
        if (value === '1' || (mode === 'edit' && is_can_update_competition === '1')){
            if(show_option === '1'){
