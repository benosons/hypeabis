						...
						<div class="bg-master-lighter padding-20 m-b-20 b-rad-lg">
							<div class="row">
								<?php if ($question->picture): ?>
									<div class="col-md-3">
										<img src="<?php echo base_url() ?>assets/quiz/question/<?php echo $question->picture ?>" class="img img-fluid img-thumbnail" />
									</div>
								<?php endif; ?>
								<div class="col-md-9">
									<h4><?php echo $question->text ?></h4>
									<p>Total penjawab: <?php echo array_sum(array_column($question->choices, 'counts')) ?></p>
								</div>
							</div>
									...
									<tbody>
										<?php foreach ($question->choices as $key => $choice): ?>
											<tr>
												...
												<td>
													<?php if ($choice->picture): ?>
														<img src="<?php echo base_url() ?>assets/quiz/choice/<?php echo $choice->picture ?>" class="img img-fluid img-thumbnail" style="max-width:150px;max-height:75px;" />
													<?php endif; ?>
												</td>
												<td><?php echo $choice->text ?></td>
												<td class="text-center">
													<?php if ($choice->is_answer === '1'): ?>
														<i class="fa fa-check"></i>
													<?php else: ?>
														<i class="fa fa-times"></i>
													<?php endif; ?>
												</td>
												<td class="text-right"><?php echo $choice->counts ?></td>
												<td class="text-right"><?php echo rtrim(rtrim(number_format($choice->percentage, 1, ',', '.'), '0'), ',') ?> %</td>
											</tr>
										<?php endforeach; ?>
									</tbody>
								</table>
